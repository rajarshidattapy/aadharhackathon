...
def compute_aflb_alerts(df: pd.DataFrame, month: Optional[str] = None, top_n: int = 20) -> pd.DataFrame:
    ...
    agg["rolling_avg_total_load"] = (
        agg.groupby(["state", "district", "pincode"])["total_load"]
        .transform(lambda s: s.rolling(window=3, min_periods=1).mean().shift(1))
    )
    denom = agg["rolling_avg_total_load"].replace(0, np.nan)
    agg["stress_score"] = (agg["total_load"] / denom).fillna(0.0).astype(float)
