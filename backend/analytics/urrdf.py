...
def compute_urrdf_alerts(df: pd.DataFrame, month: Optional[str] = None, top_n: int = 10) -> pd.DataFrame:
    ...
    agg["rolling_avg_demo_total_adult"] = (
        agg.groupby(["state", "district"])["demo_total_adult"]
        .transform(lambda s: s.rolling(window=3, min_periods=1).mean().shift(1))
    )

    # Avoid division by zero and keep float dtype stable across pandas versions.
    denom = agg["rolling_avg_demo_total_adult"].replace(0, np.nan)
    agg["inflow_score"] = (agg["demo_total_adult"] / denom).fillna(0.0).astype(float)
