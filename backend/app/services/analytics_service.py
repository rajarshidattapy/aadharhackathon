...
from backend.app.analytics.aflb import compute_aflb_alerts
from backend.app.analytics.bis import compute_bis_alerts
from backend.app.analytics.lost_generation import compute_lost_generation_alerts
from backend.app.analytics.urrdf import compute_urrdf_alerts
from backend.app.core.data_loader import get_dataset, get_merged_aadhaar_dataframe
from backend.app.schemas.biometric_alerts import (
    ...
    def predict_migration_model(self, month: Optional[str] = None, top_n: int = 10) -> pd.DataFrame:
        ...
        if self._model is None or self._thresholds is None:
            try:
                ...
                # Candidates: backend/models (new), repo/models (legacy), cwd-relative, etc.
                candidates = [
                    backend_root / "models" / "migration_score_model.pkl",
                    repo_root / "models" / "migration_score_model.pkl",
                    Path("backend") / "models" / "migration_score_model.pkl",
                    Path("models") / "migration_score_model.pkl",
                    backend_root / "migration_score_model.pkl",
                    repo_root / "migration_score_model.pkl",
                    Path("migration_score_model.pkl"),
                ]
                thresh_candidates = [
                    backend_root / "models" / "migration_thresholds.pkl",
                    repo_root / "models" / "migration_thresholds.pkl",
                    Path("backend") / "models" / "migration_thresholds.pkl",
                    Path("models") / "migration_thresholds.pkl",
                    backend_root / "migration_thresholds.pkl",
        ...
        import numpy as np
        from backend.app.utils.migration_scoring import to_inflow_score, inflow_tier, recommendations_for_tier
