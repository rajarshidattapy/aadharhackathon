...
from backend.utils.time_utils import normalize_date_column, add_month_column
from backend.utils.state_utils import normalize_state_column
...
#   repo_root/backend/core/data_loader.py -> parents[2] == repo_root
REPO_ROOT = Path(__file__).resolve().parents[2]
# Data file is located at data/merged_aadhaar_data_sample.csv
DATA_DIR = REPO_ROOT / "data"
_MERGED_CSV_ENV = os.environ.get("AADHAAR_MERGED_CSV")
_LOCAL_MERGED_CSV = DATA_DIR / "merged_aadhaar_data_sample.csv"
_MERGED_CSV_SOURCE = _MERGED_CSV_ENV if _MERGED_CSV_ENV else (
    str(_LOCAL_MERGED_CSV)
    if _LOCAL_MERGED_CSV.exists()
...
def get_merged_aadhaar_dataframe() -> pd.DataFrame:
    ...
    # Keep biometric/lost-generation state output consistent with other endpoints.
