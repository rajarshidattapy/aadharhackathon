...
# Prefer the maintained checks script under tests/, then legacy locations.
SCRIPT_CANDIDATES = [
    os.path.join(ROOT, "tests", "run_model_checks.py"),
...
SUPPORTED_EXTS = (".pt", ".pth", ".onnx", ".h5", ".keras", ".pkl")
...
def test_run_model_checks():
    if not SCRIPT or not os.path.exists(SCRIPT):
        pytest.fail("run_model_checks.py not found (looked for {})".format(", ".join(SCRIPT_CANDIDATES)))
    if not has_models():
        pytest.fail("No model files found under {}".format(MODEL_DIR))
